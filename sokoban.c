...
// Les prototypes des fonctions et procédures, détaille avant chaque fonction et procédures.
int kbhit();
...
void deplacer(t_plateau plateau, t_plateau plateauBase, char touche,
...
int main() {
  ...
  int posJoX, posJoY, nbDeplacement;
  ...
  while (jouer != NO){
    ...
    // premier affichage
    affichage_complet(plateau, nomNiveau, nbDeplacement);
    ...
    while ((touche != FIN) && (gagner == FAUX)) {
      if (kbhit()) {
        touche = getchar();
        deplacer(plateau, plateauBase, touche, &posJoX, &posJoY,
                 &nbDeplacement);
        if (touche == RELOAD) {
          charger_partie(plateau, nomNiveau);
          recherche_pos_jo(plateau, &posJoX, &posJoY);
 ...
 * @param plateauBase de type t_plateau qui est une sauvegarde du plateau de base.
...
/**
* @brief Procédure pour remplacelement de caractère "après" déplacement du joueur
* @param plateau de type t_plateau, tableau de jeu.
* @param plateauBase de type t_plateau qui est une sauvegarde du plateau de base.
* @param posJoX de type entier, sur quel ligne du tableau est le joueur,
* en entrer et sortie
...
/**
* @brief Fonction qui retourne vrai ou faux en fonction de si le joueus à gagner ou non 
* @param plateau de type t_plateau, tableau de jeu.
* @param plateauBase de type t_plateau qui est une sauvegarde du plateau de base.
*/
bool gagne(t_plateau plateau, t_plateau plateauBase) {
